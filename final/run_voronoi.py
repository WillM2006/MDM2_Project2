...
def voronoi_areas(positions: np.ndarray, tri: scipy.spatial.Delaunay) -> np.ndarray:
    """
...
def affinity_correction(
    ...
    return np.exp(-correction_strength * penalty[np.newaxis, :])
...
def combined_score(s_mass=None, s_mom=None, s_energy=None, weights=None):
    ...
    if s_mass is not None:
        available["mass"] = s_mass
    if s_mom is not None:
        available["momentum"] = s_mom
    if s_energy is not None:
        available["energy"] = s_energy
...
def compute_velocities(
    old: np.ndarray, new: np.ndarray, assignment: np.ndarray
) -> np.ndarray:
    """
...
def evaluate_assignments(
    ...
    tri = scipy.spatial.Delaunay(next_positions)
    voronoi_areas_arr = voronoi_areas(next_positions, tri)
    velocities = compute_velocities(previous_positions, next_positions, assignments)
    s_mass = score_mass(next_positions, velocities, tri, voronoi_areas_arr)
    s_mom = score_momentum(next_positions, velocities, tri, voronoi_areas_arr)
    s_energy = score_energy(next_positions, velocities, voronoi_areas_arr)
    w_mass = 1.0 / (s_mass.mean() + 1e-12)
    w_mom = 1.0 / (s_mom.mean() + 1e-12)
    w_energy = 1.0 / (s_energy.mean() + 1e-12)
    correction_strength = 0.15
    return affinity_correction(
        s_mass,
        s_mom,
        s_energy,
        w_mass,
